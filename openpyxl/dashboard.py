...
def inicializa_planilha():
    planilha = Workbook()  # cria planilha
    folha = planilha.active  # acessa folha da planilha
    folha.title = "Dashboard"  # altera nome da folha

    for coluna in "ABCDEFGH":  # ajuste de largura de coluna
        folha.column_dimensions[coluna].width = 27

    for linha in range(1, 3):  # ajuste de altura de linha
        folha.row_dimensions[linha].height = 27

    return planilha, folha
...
for i in range(num_acoes):  # Iterador das ações
    ...
    contrib = ws.cell(row=(6 + i), column=4,
...
for i in range(num_moedas):  # Iterador de moedas
    ...
    cotacao.number_format = "R$#,##0.00"  # Formato de moeda real
    contrib = ws.cell(row=(6 + i), column=8,
...
soma_val = ws.cell(row=(6 + num_linhas), column=3,
                   value="=SUM(C6:C" + str(5 + num_linhas) + ")")  # Soma dos valores das ações
soma_val.number_format = "R$#,##0.00"  # Formato de moeda real
soma_contrib = ws.cell(row=(6 + num_linhas), column=4,
                       value="=SUM(D6:D" + str(5 + num_linhas) + ")")  # Soma das contribuições das ações
...
soma_val = ws.cell(row=(6 + num_linhas), column=7,
                   value="=SUM(G6:G" + str(5 + num_linhas) + ")")  # Soma dos valores das ações
soma_val.number_format = "R$#,##0.00"  # Formato de moeda real
soma_contrib = ws.cell(row=(6 + num_linhas), column=8,
                       value="=SUM(H6:H" + str(5 + num_linhas) + ")")  # Soma das contribuições das moedas
soma_contrib.number_format = "R$#,##0.00"  # Formato de moeda real
...
soma_val = ws.cell(row=12 + num_linhas, column=4, value="=B" + str(6 + num_linhas) + "+F"
                   + str(6 + num_linhas))  # Soma das quantidades de ações e de moedas
soma_val.number_format = "R$#,##0.00"  # Formato de moeda real
soma_contrib = ws.cell(row=12 + num_linhas, column=5, value="=D" + str(6 + num_linhas) + "+H"
                       + str(6 + num_linhas))  # Soma dos valores acumulados de ações e de moedas
soma_contrib.number_format = "R$#,##0.00"  # Formato de moeda real
