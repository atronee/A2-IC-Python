...
def estrutura(lista_acoes,lista_moedas, carteira, título):  
...
def encontra_ativos(url):
    ...
    h1 = soup.find('h1')
    title = soup.find('title')      # Aqui o código vai buscar pelo título da Carteira
    if h1 is not None:              # A preferencia ao h1
        titulo = h1.text
    elif title is not None and h1 is None:
        titulo = title.text
